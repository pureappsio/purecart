...
Router.route("/api/products/:id", { where: "server" }).get(function() {
    ...
    } else if (Elements.findOne({ type: 'productPictures', productId: productId })) {
        var imageId = Elements.findOne({ type: 'productPictures', productId: productId }).imageId;
        product.image = Images.findOne(imageId).link();
    }

    if (this.params.query.variants) {

        var variants = Variants.find({ productId: productId }, { sort: { 'price.USD': 1 } }).fetch();
...
Router.route("/api/products", { where: "server" }).get(function() {
    ...
    if (this.params.query.course) {
        query.courses = this.params.query.course;
    }
...
Router.route("/api/customers", { where: "server" }).get(function() {
    ...
    if (this.params.query.lastpurchase) {
        query.lastPurchase = new Date(this.params.query.lastpurchase);
    }
